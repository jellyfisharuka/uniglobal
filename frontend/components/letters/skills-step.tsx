...
export function SkillsStep() {
    const { nextStep, setSkills, skills } = useLettersStore();
    const [value, setValue] = useState(skills || "");

    const handleSubmit = () => {
        if (!value.trim()) return;
        setSkills(value.trim());
        nextStep();
    }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-12">
            <h1 className="w-10/12 mx-auto text-xl text-center text-[#000066]">
                Какое качество или навык, по вашему мнению, помогает вам добиваться успеха?
            </h1>
            ...
            <div className="w-10/12 mx-auto text-sm text-gray-600">
                ...
                <p className="leading-relaxed text-[12px]">
                    Я считаю, что моей главной силой является упорство. Когда я сталкиваюсь с трудностями, я не сдаюсь и ищу новые способы решения проблемы, пока не найду решение.
                </p>
            </div>
        </div>
    )
